  ...
  created() {
    if (
      ...
      if (code) {
        ...
        this.$axios
          .get("OAuth2/authorize?name=bankCardDG&area=DG&code=" + code)
          .then((res) => {
            let datas = res.data;
            if (datas.code === 1) {
              console.log("授权成功");
  ...
  methods: {
    afterUploaderimg(file) {
      ...
      image.onload = function () {
        ...
        let param = {
          imgFile: base64,
          empNo: _this.$store.getters.empNo,
        };
        axios
    ...
    onSubmit() {
      if (
        ...
        Dialog.confirm({
          ...
          if (this.empSalaryGroup == "D2" && this.bankCode != "ZCN20") {
            Toast.fail("暂不支持工商以外的银行卡");
          } else if (this.empSalaryGroup != "D2" && this.bankCode != "ZCN10") {
            Toast.fail("暂不支持邮储以外的银行卡");
          } else {
            let record = {
              workNo: this.workNo,
